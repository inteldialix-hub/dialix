...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    const agents = isAdmin
      ? all('SELECT DISTINCT agent_id, agent_name FROM client_agents')
      : all('SELECT agent_id, agent_name FROM client_agents WHERE client_id = ?', [clientId]);

    // Get phone numbers count
    const phoneNumbers = isAdmin
      ? all('SELECT COUNT(*) as count FROM phone_numbers')
      : all('SELECT COUNT(*) as count FROM phone_numbers WHERE client_id = ?', [clientId]);
    const totalPhoneNumbers = phoneNumbers[0]?.count || 0;

    // Get client count (admin only)
    let totalClients = 0;
    if (isAdmin) {
      const clientCount = get('SELECT COUNT(*) as count FROM clients WHERE is_admin = 0');
    ...
    for (const agent of agents) {
      try {
        ...
        // Agent might not have any conversations yet
        callsByAgent.push({
          agent_id: agent.agent_id,
          name: agent.agent_name,
          count: 0,
