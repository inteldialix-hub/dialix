...
const BASE_URL = 'https://www.inteldialix.online';

function loadCategory(name) {
  const filePath = path.join(AI_DIR, 'src', 'data', 'seo', `${name}.ts`);
...
const integrations = loadCategory('integrations');
const solutions = loadCategory('solutions');
const comparisons = loadCategory('comparisons');
const templates = loadCategory('templates');

console.log(`Loaded: ${integrations.length} integrations, ${solutions.length} solutions, ${comparisons.length} comparisons, ${templates.length} templates.`);

// Generate llms.txt
const topIntegrations = integrations.slice(0, 8);
const topSolutions = solutions.slice(0, 4);
const topComparisons = comparisons.slice(0, 3);
const topTemplates = templates.slice(0, 2);

const llmsTxtContent = `# Dialix Voice AI Platform
...
const llmsFullTxtContent = `# Dialix Voice AI Platform — Comprehensive Technical Documentation & RAG Feed
...
fs.writeFileSync(path.join(AI_DIR, 'public', 'llms.txt'), llmsTxtContent, 'utf-8');
fs.writeFileSync(path.join(AI_DIR, 'public', 'llms-full.txt'), llmsFullTxtContent, 'utf-8');

console.log('Successfully wrote public/llms.txt (bytes:', Buffer.byteLength(llmsTxtContent), ')');
console.log('Successfully wrote public/llms-full.txt (bytes:', Buffer.byteLength(llmsFullTxtContent), ')');
