...
import { FALLBACK_LLM_OPTIONS, FALLBACK_TTS_MODEL_OPTIONS, FALLBACK_LANGUAGE_OPTIONS } from '@/lib/constants';
...
export default function AgentDetailPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      await api(`/agents/${agentId}`, {
        ...
        body: {
          name, first_message: firstMessage, language, disable_first_message_interruptions: disableFirstMsgInterrupt,
          prompt, llm, temperature, max_tokens: maxTokens, ignore_default_personality: ignoreDefaultPersonality,
  ...
  const Toggle = ({ value, onChange, label, desc }: { value: boolean; onChange: (v: boolean) => void; label: string; desc?: string }) => (
    <div className="form-group" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 12 }}>
      ...
      <div className={`toggle ${value ? 'active' : ''}`} onClick={() => onChange(!value)} style={{ cursor: 'pointer', width: 40, height: 22, borderRadius: 11, background: value ? 'var(--accent)' : 'var(--bg-tertiary)', position: 'relative', transition: 'background 0.2s' }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div className="detail-header">
        ...
        <div style={{ flex: 1 }} />
        <SneakyButton text="Test Call" onClick={promptTestCall} className="button06--green" />
        <SneakyButton text={saving ? 'Saving...' : 'Save Changes'} onClick={handleSave} className={saving ? '' : ''} />
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 32px', borderBottom: '1px solid var(--border-default)', background: 'var(--bg-secondary)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2, width: 200 }}>
          ...
          <select className="voice-select" style={{ padding: '6px 10px', fontSize: 13, background: 'var(--bg-input)', color: 'var(--text-primary)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-md)' }} value={callPhoneId} onChange={e => setCallPhoneId(e.target.value)}>
            <option value="">Select...</option>
            {phoneNumbers.filter(p => !p.assigned_agent_id || p.assigned_agent_id === agentId).map(p => <option key={p.id} value={p.id}>{p.label}</option>)}
          </select>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2, flex: 1 }}>
          <span style={{ fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--text-quaternary)' }}>Phone Number</span>
        ...
        <SneakyButton text={calling ? 'Calling...' : 'Call Now'} onClick={handleCall} className="button06--green" />
      </div>
        ...
        <div className="config-panel">
          ...
          {activeTab === 'general' && (<>
            <div className="config-section">
              ...
              <div className="form-group" style={{ marginTop: 12 }}><label className="form-label">Language</label><CustomSelect value={language} onChange={e => set(setLanguage)(e.target.value)} options={liveLanguages} /></div>
              <div className="form-group" style={{ marginTop: 12 }}><label className="form-label">First Message</label><div style={{ fontSize: 11, color: 'var(--text-quaternary)', marginBottom: 4 }}>The agent&apos;s opening greeting. Use {'{{lead_name}}'} for dynamic names.</div><textarea className="form-input" value={firstMessage} onChange={e => set(setFirstMessage)(e.target.value)} rows={3} style={{ resize: 'vertical' }} /></div>
              <Toggle value={disableFirstMsgInterrupt} onChange={set(setDisableFirstMsgInterrupt)} label="Block Interruptions" desc="Prevent users from interrupting the first message." />
            </div>
          </>)}
          ...
          {activeTab === 'prompt' && (<>
            <div className="config-section">
              ...
              <div className="form-group"><label className="form-label">LLM</label><CustomSelect value={llm} onChange={e => set(setLlm)(e.target.value)} options={liveLLMs} /></div>
              ...
              <Toggle value={ignoreDefaultPersonality} onChange={set(setIgnoreDefaultPersonality)} label="Ignore Default Personality" desc="Disables ElevenLabs default personality traits." />
            </div>
          </>)}
                ...
                <>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                    ...
                    <label style={{ position: 'relative', display: 'inline-block', width: 42, height: 24, cursor: 'pointer' }}>
                      ...
                      <span style={{ position: 'absolute', inset: 0, borderRadius: 12, background: expressiveMode ? 'var(--accent-primary, #6366f1)' : 'var(--bg-tertiary, #374151)', transition: 'background 0.2s' }}>
                    ...
                    <div style={{ marginBottom: 16 }}>
                      ...
                      {audioTags.length > 0 && (
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 }}>
                          {audioTags.map((tag, i) => (
                            <span key={i} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 10px', fontSize: 12, fontWeight: 500, background: 'rgba(99, 102, 241, 0.12)', color: 'var(--accent-primary, #818cf8)', border: '1px solid rgba(99, 102, 241, 0.3)', borderRadius: 16, cursor: 'default' }}>
                              {tag.name}
                ...
                <div style={{ display: 'flex', gap: 0 }}>
                  {[0, 1, 2, 3, 4].map(v => (
                    <button key={v} type="button" onClick={() => set(setStreamingLatency)(v)} style={{
                      ...
                      background: streamingLatency === v ? 'var(--accent)' : 'var(--bg-tertiary)',
          ...
          {activeTab === 'call-behavior' && (<>
            <div className="config-section">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <div className="form-group"><label className="form-label">Turn Timeout (sec)</label><input className="form-input" type="number" value={turnTimeout} onChange={e => set(setTurnTimeout)(parseFloat(e.target.value))} min="0.5" max="10" step="0.5" /></div>
                ...
                <div className="form-group"><label className="form-label">Turn Eagerness</label><CustomSelect value={turnEagerness} onChange={e => set(setTurnEagerness)(e.target.value)} options={[{ value: 'low', label: 'Low' }, { value: 'normal', label: 'Normal' }, { value: 'high', label: 'High' }]} /></div>
              </div>
              <div className="form-group" style={{ marginTop: 12 }}><label className="form-label">Mode</label><CustomSelect value={mode} onChange={e => set(setMode)(e.target.value)} options={[{ value: 'turn', label: 'Turn-based' }, { value: 'off', label: 'Off (free-form)' }]} /></div>
            </div>
          </>)}
      ...
      {/* Test Call — ElevenLabs-style full-screen overlay */}
      {showTestCall && token && (
        <TestCallView
          agentId={agentId}
