...
export default function AdminPage() {
  ...
  return (
    <div className="layout-container" style={{ padding: '40px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <div className="config-panel" style={{ padding: '0', overflow: 'hidden', border: '1px solid var(--border-default)', background: 'var(--bg-raised)', borderRadius: '12px' }}>
        <div className="config-tabs" style={{ padding: '0 24px', borderBottom: '1px solid var(--border-default)', marginBottom: 0 }}>
          <div className={`config-tab ${tab === 'clients' ? 'active' : ''}`} onClick={() => setTab('clients')} style={{ padding: '16px 8px' }}><Icon name="users" size={14} /> Clients ({clients.length})</div>
          <div className={`config-tab ${tab === 'agents' ? 'active' : ''}`} onClick={() => setTab('agents')} style={{ padding: '16px 8px' }}><Icon name="bot" size={14} /> Agents ({allAgents.length})</div>
        </div>
                        ...
                        <div style={{ padding: '24px', background: 'var(--bg-input)', borderTop: '1px solid var(--border-default)', borderBottom: '1px solid var(--border-default)' }}>
                                    ...
                                    <div key={ag.agent_id} style={{
                                      ...
                                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                                          ...
                                          <span style={{ fontSize: '12px', color: 'var(--text-tertiary)', fontFamily: 'var(--font-mono)' }}>{ag.agent_id.slice(0, 16)}...</span>
                                          ...
                                          {disabledCount > 0 && (
                                            <span style={{
                                              display: 'inline-flex', alignItems: 'center', gap: '4px',
                                              fontSize: '11px', fontWeight: 600, padding: '3px 8px',
                                              borderRadius: '6px', background: 'rgba(239,68,68,0.1)', color: '#ef4444',
                                            }}>
                                        ...
                                        <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                                          {/* Features Button (gear) */}
                                          <button
                                            className={`btn-ghost ${isFeatureExpanded ? 'active' : ''}`}
                                            onClick={(e) => {
                                              e.stopPropagation();
                                              setExpandedFeatureAgent(isFeatureExpanded ? null : { clientId: c.id, agentId: ag.agent_id });
                                            }}
                                            title="Manage visible features"
                                            style={{ color: isFeatureExpanded ? 'var(--brand-accent)' : 'var(--text-secondary)', padding: '6px 10px', fontSize: '12px' }}
                                          >
                                            <Icon name="settings" size={14} /> Features
                                          </button>
                                          {/* Toggle Permission Button (lock) */}
                                          <button
                                            className="btn-ghost"
                                            ...
                                            style={{ color: 'var(--text-secondary)', padding: '6px 10px', fontSize: '12px' }}
                                          >
                                            <Icon name={ag.can_edit ? 'lock' : 'unlock'} size={14} /> {ag.can_edit ? 'Lock' : 'Unlock'}
                                          </button>
                                          {/* Remove Button */}
                                          <button
                                            className="btn-ghost"
                                            ...
                                            title="Remove agent"
                                            style={{ color: 'var(--danger)', padding: '6px 10px', fontSize: '12px' }}
                                          >
                                      ...
                                      {/* Feature Toggles Panel — inline expandable */}
                                      {isFeatureExpanded && (
                                        <div style={{ borderTop: '1px solid var(--border-default)', padding: '16px', background: 'var(--bg-base)' }}>
                          ...
                          <div style={{ padding: '16px', background: 'var(--bg-overlay)', borderRadius: '8px', border: '1px solid var(--border-default)' }}>
                            ...
                            {getAvailable(c.id).length === 0 ? (
                              ...
                              <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-end', position: 'relative', zIndex: 50 }}>
                                <div style={{ flex: 1, position: 'relative', zIndex: 51 }}>
                                ...
                                <div style={{ width: '160px', flexShrink: 0, position: 'relative', zIndex: 50 }}>
                                ...
                                <button className="btn-primary" style={{ whiteSpace: 'nowrap', height: '40px', padding: '0 20px' }} onClick={() => handleAssignAgent(c.id)} disabled={!selectedAgentToAssign}>
          ...
          {tab === 'agents' && (<>
            ...
            {allAgents.length === 0 ? <EmptyState icon="bot" title="No Agents" description="Create an agent from the Agents page first." /> : (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '16px' }}>
                {allAgents.map((a) => (
                  <div key={a.agent_id} className="agent-card" style={{ padding: '20px', borderRadius: '12px', border: '1px solid var(--border-default)', background: 'var(--bg-raised)', display: 'flex', flexDirection: 'column', gap: '16px', transition: 'all 0.2s', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      <div style={{ width: '48px', height: '48px', borderRadius: '12px', background: `linear-gradient(135deg, hsl(${(a.name || '').charCodeAt(0) * 37 % 360}, 70%, 50%), hsl(${(a.name || '').charCodeAt(0) * 53 % 360}, 60%, 40%))`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', fontWeight: 700, color: '#fff', boxShadow: '0 2px 8px rgba(0,0,0,0.2)' }}>
                        {(a.name || '?')[0].toUpperCase()}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                          <span style={{ fontWeight: 600, fontSize: '15px', color: 'var(--text-primary)' }}>{a.name}</span>
                          <span className={`provider-badge provider-${a.provider || 'elevenlabs'}`}>{a.provider === 'vapi' ? 'Vapi' : 'ElevenLabs'}</span>
                        </div>
                        <div style={{ fontSize: '12px', color: 'var(--text-tertiary)', fontFamily: 'var(--font-mono)' }}>{a.agent_id.slice(0, 24)}...</div>
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: '10px', marginTop: 'auto', paddingTop: '16px', borderTop: '1px solid var(--border-default)' }}>
                      <button className="btn-ghost" style={{ flex: 1, fontSize: '13px', padding: '8px 12px', background: 'var(--bg-overlay)' }} onClick={() => router.push(`/dashboard/agents/${a.agent_id}`)}><Icon name="settings" size={14} /> Configure</button>
                      <button className="btn-ghost" style={{ fontSize: '13px', padding: '8px 12px', color: 'var(--danger)', background: 'var(--bg-overlay)' }} onClick={() => setDeleteAgentTarget(a)} title="Delete Agent"><Icon name="trash-2" size={14} /></button>
                    </div>
                  </div>
                ))}
              </div>
