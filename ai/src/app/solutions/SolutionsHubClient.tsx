...
import { Search, ArrowRight, Layers, Sparkles, PhoneCall, CheckCircle2 } from 'lucide-react';

interface SolutionsHubClientProps {
  solutions: ProgrammaticPageData[];
}

export default function SolutionsHubClient({ solutions }: SolutionsHubClientProps) {
  const [search, setSearch] = useState('');
  const [selectedCategory, setSelectedCategory] = useState<string>('All');

  const categories = useMemo(() => {
    const cats = new Set(solutions.map((s) => s.category));
  ...
  }, [solutions, search, selectedCategory]);
      ...
      <main className="flex-1 w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-16">
        ...
        <header className="mt-4 mb-10 text-center max-w-3xl mx-auto">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full text-xs font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 mb-4">
            ...
            <span>100+ Enterprise Voice AI Solutions</span>
          </div>

          <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold tracking-tight text-white mb-4 leading-tight">
            Industry & Telephony Voice AI Solutions
          </h1>

          <p className="text-base sm:text-lg text-zinc-300 leading-relaxed font-normal">
            Automate high-volume phone operations across healthcare, real estate, financial services, logistics, and retail. Eliminate wait times, cut call center overhead, and deliver human-grade conversational experiences.
          </p>
          ...
          <div className="mt-8 relative max-w-xl mx-auto">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search 100+ solutions (e.g. Inbound Support, Dental, Speed to Lead, HVAC)..."
        ...
        <section className="my-10">
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-16 rounded-2xl bg-[#111317] border border-white/[0.08]">
              ...
              <h3 className="text-base font-semibold text-white mb-1">No solutions found</h3>
              ...
              {filtered.map((item) => (
                ...
                >
                  ...
                  <div className="pt-3 border-t border-white/[0.04] flex items-center justify-between text-[11px] text-zinc-500 font-mono">
                    ...
                    <span className="text-emerald-400 font-medium">{item.benchmarks[0]?.value}</span>
                  </div>
                </Link>
              ))}
            </div>
