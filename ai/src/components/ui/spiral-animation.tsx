...
class Star {
    ...
    render(p: number, controller: AnimationController) {
        ...
        if (q > 0) {
            ...
            // Convert 2D screen coordinates to 3D space
            // Access private fields via bracket notation for the projection math
            const camZ = -400;
            const viewZ = 100;
            const vx = (this.z - camZ) * screenX / viewZ;
            const vy = (this.z - camZ) * screenY / viewZ;
...
export function SpiralAnimation() {
    ...
    // SSR-safe: don't access window during server render
    const [dimensions, setDimensions] = useState({ width: 0, height: 0 })
    
    // Set initial dimensions on mount (client only)
    useEffect(() => {
        const handleResize = () => {
            setDimensions({
